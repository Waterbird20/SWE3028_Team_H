...
class FinanceParser:
    ...
    def parse_data_args(self):
        return DataArgs(stock_type=self.arg['stock_type'], seq_length=self.arg['seq_length'], output_length=self.arg['output_length'], predict_type=self.arg['predict_type'])
    ...
    def parse_transformer_args(self):
        return TransformerArgs(embed_dim=self.arg['embed_dim'], resolution=self.arg['resolution'], n_head=self.arg['n_head'], n_layer=self.arg['n_layer'], fc_hidden_size=self.arg['fc_hidden_size'], output_length=self.arg['output_length'], seq_length=self.arg['seq_length'])
